 ...
 */
@Component
public class AfrinicObjectTypeFactory implements IObjectTypeFactory {

    public static final int DUMMY_OBJECT_TYPE_ID = 100;

    private final Map<String, IObjectType> TYPE_NAMES;
    private final Set<IObjectType> SET_OBJECTS;
    private final Map<Integer, IObjectType> BY_TYPE_ID;
    private static final Map<Class<? extends IObjectType>, IObjectType> TYPE_CLASSES = Maps.newHashMap();

    @Autowired
    public AfrinicObjectTypeFactory(final List<IObjectType> objectTypes) {
    ...
    private void insertInTypeClasses(IObjectType type) {
        TYPE_CLASSES.put(type.getClass(), type);
    }
